...
import { ModulePanels, PanelParams } from '@/constant';
import { DetailDashboard } from '@/components';

export const DetailDashboardCompactPanel: React.FC<LayoutPanelComponent> = ({ layoutService }) => {
    ...
    const _handleToggleFullscreen = (aspect_key: string) => {
        const paramsQ = layoutService.getParam('primary');
        const paramsQS = layoutService.getParam('secondary');
        const paramsQE = layoutService.getParam('extension');

        layoutService
            .setLayout(aspect_key)
